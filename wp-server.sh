...
function print_usage() {
  ...
  echo -e "  ${GREEN}max-upload [-m <MB>]${RESET}   Set PHP upload and Nginx max body size for the current site. If -m is not provided, the value will be read from .user.ini or prompted."
...
case "$COMMAND" in
  ...
  cache)
  if ! $SPINUPWP_ACTIVE; then
    echo "Error: Cache commands require a WordPress management plugin to be active."
    exit 1
  fi
  if [ -z "$CURRENT_USER" ] || [ -z "$USER_HOME_PATH" ]; then
  ...
  if [ -n "$SET_UPLOAD_VALUE" ]; then
    ...
    if [ -z "$UPLOAD_VALUE" ]; then
      read -p "Enter desired upload size in MB: " USER_INPUT_UPLOAD
      # Validate input
  ...
  # Only create/update .user.ini if it doesn't exist or doesn't have upload_max_filesize
  SHOULD_UPDATE_INI=false
  if [ ! -f "$USER_INI_PATH" ]; then
    SHOULD_UPDATE_INI=true
  elif ! grep -q "^upload_max_filesize" "$USER_INI_PATH"; then
    SHOULD_UPDATE_INI=true
  elif [ -n "$SET_UPLOAD_VALUE" ]; then
    # If -m flag was provided, we should update .user.ini
    SHOULD_UPDATE_INI=true
  fi

  if [ "$SHOULD_UPDATE_INI" = true ]; then
    mkdir -p "$(dirname "$USER_INI_PATH")" || {
        echo "ERROR: Failed to create directory for .user.ini file"
        exit 1
    }

    # Calculate post_max_size (upload + 2MB)
    POST_MAX_SIZE=$((UPLOAD_VALUE + 2))

    # Check current values
    CURRENT_INI_UPLOAD=""
    CURRENT_INI_POST=""
    if [ -f "$USER_INI_PATH" ]; then
      CURRENT_INI_UPLOAD=$(grep "^upload_max_filesize" "$USER_INI_PATH" | cut -d'=' -f2 | tr -d ' ' | sed 's/[^0-9]*//g')
      CURRENT_INI_POST=$(grep "^post_max_size" "$USER_INI_PATH" | cut -d'=' -f2 | tr -d ' ' | sed 's/[^0-9]*//g')
    fi

    # Update or add upload_max_filesize
    if [ -n "$CURRENT_INI_UPLOAD" ] && [ "$CURRENT_INI_UPLOAD" -eq "$UPLOAD_VALUE" ]; then
      echo "PHP upload_max_filesize is already ${UPLOAD_VALUE}M. Skipping update."
    elif grep -q "^upload_max_filesize" "$USER_INI_PATH" 2>/dev/null; then
      if sed -i "s/^upload_max_filesize =.*/upload_max_filesize = ${UPLOAD_VALUE}M/" "$USER_INI_PATH"; then
        echo "Updated upload_max_filesize in .user.ini to ${UPLOAD_VALUE}M."
      else
        echo "ERROR: Failed to update .user.ini file"
        exit 1
      fi
      ...
      if echo "upload_max_filesize = ${UPLOAD_VALUE}M" >> "$USER_INI_PATH"; then
        echo "Added upload_max_filesize to .user.ini with value ${UPLOAD_VALUE}M."
      else
        echo "ERROR: Failed to write to .user.ini file"
        exit 1
      fi
    ...
    if [ -n "$CURRENT_INI_POST" ] && [ "$CURRENT_INI_POST" -eq "$POST_MAX_SIZE" ]; then
      echo "PHP post_max_size is already ${POST_MAX_SIZE}M. Skipping update."
    elif grep -q "^post_max_size" "$USER_INI_PATH" 2>/dev/null; then
      if sed -i "s/^post_max_size =.*/post_max_size = ${POST_MAX_SIZE}M/" "$USER_INI_PATH"; then
        echo "Updated post_max_size in .user.ini to ${POST_MAX_SIZE}M."
      else
        echo "ERROR: Failed to update .user.ini file"
        exit 1
      fi
    else
      if echo "post_max_size = ${POST_MAX_SIZE}M" >> "$USER_INI_PATH"; then
        echo "Added post_max_size to .user.ini with value ${POST_MAX_SIZE}M."
      else
        echo "ERROR: Failed to write to .user.ini file"
        exit 1
      fi
    ...
    echo "Using existing upload_max_filesize from .user.ini. Not modifying .user.ini."
  fi
  ...
  if [ -n "$NGINX_CURRENT_UPLOAD" ] && [ "$NGINX_CURRENT_UPLOAD" -eq "$UPLOAD_VALUE" ]; then
    echo "Nginx client_max_body_size is already ${UPLOAD_VALUE}m. Skipping update."
  else
    mkdir -p "$(dirname "$NGINX_CONF_PATH")" || {
        echo "ERROR: Failed to create directory for nginx config file"
        exit 1
